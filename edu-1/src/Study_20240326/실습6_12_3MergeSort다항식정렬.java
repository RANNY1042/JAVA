...
public class 실습6_12_3MergeSort다항식정렬 {
	...
	static void merge(Polynomial3[] a, int lefta, int righta, int leftb, int rightb) {
		...
		int i = lefta, j = leftb, k = 0; // 배열 인덱스
	...
	static void ShowPolynomial(String str, Polynomial3[] x, int count) {
		...
		// count가 -1이면 다항식 x의 length로 계산하고 -1이면 아니면 count가 다항식 항의 숫자이다
	...
	static int AddPolynomial(Polynomial3[] x, Polynomial3[] y, Polynomial3[] z) {
		...
		// x배열과 y배열 merge
		// 1. x의 exp와 y의 exp가 같을때 add(coef)

		while (p < x.length && q < y.length) {
			if (x[p].exp == y[p].exp) {
				z[r].coef = x[p].coef + y[p].coef;
				z[r].exp = x[p].exp;
				p++;
				q++;
				r++;
		...
		// 2. x의 exp와 y의 exp가 다를때 sort해서 나타냄
		while (p < x.length) {
			z[r] = x[p];
			p++;
			r++;
	...
	static int addTerm(Polynomial3[] z, Polynomial3 term, int terms) {
		...
		int r=0 ;
		if(z[terms].exp==z[r].exp) {
			for(i=0;i<z.length;i++) {
				z[r]=z[terms].coef+z[i].coef;
			}
		...
		static int MultiplyPolynomial(Polynomial3[]x,Polynomial3[]y,Polynomial3[]z) {
			...
			for (int p=0;) {
				for(int q=0;) {
					...
					newTerm.exp = x[p].exp + y[q].coef; }
				
					addTerm(z,newTerm,terms++); //작은함수를 만들고 이를 이용해서 구현
				}
